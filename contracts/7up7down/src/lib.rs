...
impl SlotMachine {
    ...
    pub fn play(&mut self, choice: u8) -> HumanReadableGameResult {
        ...
        let mut gen_choice: u8 = 3;
        let rand: u8 = *env::random_seed().get(0).unwrap();
        let mut reward: u128 = ONE_NEAR * 5;
        let break7: u8 = 118;
        if rand < break7 {
            gen_choice = 1;
            reward = ONE_NEAR * 2;
        } else if rand < (break7 + 20) {
            gen_choice = 2;
            reward = ONE_NEAR * 2;
        }
        ...
        if choice == gen_choice {  // He wins
            result.reward_amount = reward.into();
            Promise::new(account_id.clone()).transfer(reward.into());
        }
        credits = 0;
        self.credits.insert(&account_id, &credits);
        result
